...
아직 이 개선판의 Pi 절감 수치는 없습니다. 구현/CI와 실제 Pi 실측을 구분합니다.
현재 운영 배포는 기존 배포용 작업이 소유하며, 직접 sudo 인증이 가능한 시점에만 비교합니다.
...
화면·page 순서·전용 프로필 여부·주변 부하를 함께 보관합니다. 아래 항목은 phase별
중앙값과 최소/최대, 실패 횟수를 보고합니다.
