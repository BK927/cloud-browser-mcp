...
**현재: 외부 계약 0.3 초안을 적용한 개발판.** 기존 10개 도구에 WebMCP 확장 2개를
...
- 알려진 민감 입력/iframe 화면은 기본적으로 이미지를 거부합니다. 운영자가 선택한
  `CB_IFRAME_SCREENSHOT_POLICY=mask`에서는 제한된 viewport 마스킹만 허용합니다.
...
- WebMCP는 실제 Chromium 지원이 필요합니다. `CB_WEBMCP_TESTING=true`는 운영자가
  ...
  미지원 엔진은 명시적 오류를 반환합니다. 모든 호출은 사용자 승인을 요구합니다.
- 동일 출처의 접근 가능한 iframe은 본문을 읽습니다. iframe 내부 자동 조작과 passkey/보안 키
  전달은 제공하지 않으며 필요한 경우 수동 제어 또는 지원 불가 결과로 처리합니다.
